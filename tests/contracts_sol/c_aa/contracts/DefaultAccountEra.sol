...
import {IAccount, ACCOUNT_VALIDATION_SUCCESS_MAGIC, Transaction} from "./IAccount.sol";
import {Utils} from "./Utils.sol";
import {INonceHolder} from "./INonceHolder.sol";

address payable constant BOOTLOADER_FORMAL_ADDRESS = payable(address(0x8001));

address constant NONCE_HOLDER_SYSTEM_CONTRACT_ADDRESS = address(0x8003);
...
//        }
    ...
    function _incrementNonce(uint256 _tx_nonce) internal {
      address to_ = NONCE_HOLDER_SYSTEM_CONTRACT_ADDRESS;
      bytes memory data = abi.encodeCall(INonceHolder.incrementMinNonceIfEquals, (_tx_nonce));
      uint256 size;
      bool success;
      uint256 gasToUse = gasleft() / 2;
      uint32 gas_ = Utils.safeCastToU32(gasToUse);
      assembly {
        success := call(gas_, to_, 0, add(data, 0x20), mload(data), 0, 0)
        size := returndatasize()
      }
      if (!success) {
    ...
    function _execute(Transaction calldata _transaction) internal {
        ...
        uint32 gas_ = Utils.safeCastToU32(gasleft());
...
//        }
    ...
    // Deployment
        if (_transaction.reserved[1] != 0 && _transaction.to == 0) {
            address newContract;
            assembly {
                newContract := create(value, add(data, 0x20), mload(data))
            }
            require(newContract != address(0), "Contract creation failed");
        } else {
            assembly {
               let success := call(gas_, to, value, add(data, 0x20), mload(data), 0, 0)
                if iszero(success) {
                    revert(0, 0)
                }
            }
